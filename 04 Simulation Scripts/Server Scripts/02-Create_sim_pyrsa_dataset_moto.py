...
def stim_id_to_label(stim_ids, label_dict, n_stim=None, crop=False):
    ...
    if stim_ids["RegressorNames"][0][0:2] != "n0":
        for i in range(n_stim):
            synset = (
                "n0" + stim_ids["RegressorNames"].astype(str)[:n_stim][i].split(".")[0]
            )
            if crop:
...
def main(sub=1):
    ...
    voxel_ids_dict = mask_utils.load_dict(
        os.path.join(
            mask_dir, "sub-" + str(sub).zfill(2) + "_voxel_IDs_dict_EPI_disjoint.npy"
        )
    )
        ...
        for perm in perm_range:
            ...
            for roi_h in mask_dict.keys():
                ...
                if estimate_cronbach and processing_mode in ["datasets", "both"]:
                    cronbach_alphas = batch_cronbachs_alpha(
                        measurements_cleaned, stimulus_descriptors
                    )
                    cronbachs_list.append(cronbach_alphas)
                    print(
                        "For",
                        roi_h,
                        "the mean internal consistency of all" + " stimuli is:",
                        np.mean(cronbach_alphas),
                    )
                ...
                if save_dataset:
                    if processing_mode in ["datasets", "both"]:
                        ...
                        dataset = rsatoolbox.data.Dataset(
                            ...
                            channel_descriptors={
                                "positions": voxel_ids_dict_tmp[roi_h]
                            },
                        )
                        ...
                        print("Created rsatoolbox dataset:", dataset_filename)
