...
# -*- coding: utf-8 -*-
"""
Utility funtions for creating ROI masks from an atlas in native space and
applying them to fMRI images
...
def merge_masks(atlas_o, roi_ids, merge_list, merged_names, rh_constant=200):
    ...
    This function assumes that IDs for right hemisphere ROIs are a function of IDs
    for left hemisphere ROIs
    ...
    for sublist in merge_list:
        ...
        for other_roi in sublist[1:]:
            ...
            if rh_constant is not None:
                atlas_array[atlas_array == rest_id + rh_constant] = (
                    ref_id + rh_constant
...
def merge_freesurfer_masks(mask_dict_raw, merge_list, merged_names):
    ...
    assert len(mask_dict) == len(mask_dict_raw) - m + 2 * len(merged_names), (
        "Unexpected number of masks in final mask_dict. Check,"
        + " if merged_names are different from strings in merge_list."
    )
...
def make_roi_mask(atlas, betas_example, roi_id, fwhm=None, interpolation="nearest"):
    ...
    Args:
        ...
        betas_example (Nifti1Image):
            Example image of GLM coefficients which will be masked later
            (only needed for shape and affine)
        roi_id (int):
        ...
        interpolation (str):
            Interpolation method by nilearn.image.resample_img
            (consult nilearn documentation for other options)
    ...
    # Resample mask
    mask_nifti_r = image.resample_img(
        mask_nifti_s,
...
def threshold_mask(mask_EPI, threshold=0.5, voxel_dimensions=None, mask_T1w=None):
    ...
    assert (
        type(threshold) == float or threshold == "adaptive"
    ), "threshold must be floating point number or 'adaptive'."
    if type(threshold) == float:
        assert (
            threshold >= 0 and threshold <= 1
        ), "numeric threshold must be between 0 and 1."
        assert (
            np.unique(mask_array)[1] < threshold
        ), "numeric threshold set too low, all values will pass."
    else:
        assert (
            type(voxel_dimensions) == dict
        ), "for adaptive thresholding you must provide the voxel dimensions in mm."
    ...
    if type(threshold) == float:
        ...
        mask_thresholded = optimize_threshold(
            mask_array, geom_vol_T1w, voxel_dimensions
        )
...
def optimize_threshold(
    ...
    while abs(delta) > epsilon and i <= max_iter:
        ...
        geom_vol_EPI = sum(sum(sum(mask_thresholded))) * np.prod(
            voxel_dimensions["EPI"]
        )
...
def remove_mask_overlap(mask_dict_r, mask_dict_t):
    """
    Find overlapping voxels in thresholded masks and only keep the voxel
    with highest pre-threshold voxel intensity
    ...
    for idx_number in range(len(overlap_indices)):
        ...
        # Remove the ROI with higher voxel intensity from overlap list
        # and set the voxel to 0 for all remaining entries
...
def create_mask_dicts(
    ...
    Args:
        ...
        betas_example (Nifti1Image):
            Example image of GLM coefficients which will be masked later
            (only needed for shape and affine)
            Note: The created masks are not specific to the predictor
            whose weight is estimated by the beta coefficients
    ...
    for roi in roi_ids.keys():
        for side in hemi_dict.keys():
            ...
            mask_nifti_r, mask_nifti_s, mask_nifti_o = make_roi_mask(
                atlas, betas_example, roi_id, fwhm=fwhm, interpolation=interpolation
            )
            mask_nifti_t = threshold_mask(
                mask_nifti_r,
                threshold=threshold,
                voxel_dimensions=voxel_dimensions,
                mask_T1w=mask_nifti_o,
            )
...
def load_dict(dict_path, merge_list=None, merged_names=None):
    """
    Load a previously saved mask dictionary.
    Optionally test if it still matches your target ROIs
    (in case you have changed your merge_list)
    ...
    if merge_list is not None and merged_names is not None:
        intersect_bad, intersect_good = test_roi_id_overlap(
            loaded_dict, merge_list, merged_names
        )
        if len(intersect_bad) > 0 or len(intersect_good) < len(merged_names):
            print(
                "Warning: Loaded mask dictionary contains masks that still"
                + " require merging. You create a new one."
            )
    return loaded_dict
...
def apply_roi_mask_SPM(glm_dir, img_num, mask, target="betas", method="nilearn"):
    """
    Load 3-D image for prespecified beta-coefficient or residual
    and apply mask to get a pattern vector
    This function assumes that beta and residual images are
    in the same directory and follow the SPM naming convention
    ...
    if method == "nilearn":
        masked = masking.apply_mask(
            image, mask, dtype="f", smoothing_fwhm=None, ensure_finite=True
        )
    elif method == "custom":
...
def mask_and_get_SPM_measurements(mask, glm_dir, n_stim=None, method="nilearn"):
    """
    Apply mask to all beta images of interest and store them in a measurements array
    (later input for pyrsa.dataset())
    ...
    for stim_num in range(n_stim):
        beta_vector = apply_roi_mask_SPM(
            glm_dir, stim_num + 1, mask, target="betas", method=method
        )
...
def mask_and_get_SPM_residuals(mask, glm_dir, n_res=None, method="nilearn"):
    """
    Apply mask to all residual images and store them in an array
    (used for estimating the precision matrix for crossnobis distance)
    ...
    for res_num in range(n_res):
        res_vector = apply_roi_mask_SPM(
            glm_dir, res_num + 1, mask, target="residuals", method=method
        )
...
def apply_roi_mask_4d(image, mask, brain_mask_path=None, method="nilearn"):
    """
    Apply mask to a 4d image and store resulting vectors in a measurements array
    (later input for pyrsa.dataset())
    ...
    if method == "nilearn":
        ...
        masked = masking.apply_mask(
            image_tmp, mask, dtype="f", smoothing_fwhm=None, ensure_finite=True
        )
    elif method == "custom":
        # Alternative to nilearn's implementation (allows for plotting of masked 3d image)
        mask_array = mask.get_fdata()
...
def get_roi_ids_glasser(txt_dir, target_ROIs):
    ...
    roi_ids_tab["ROI"] = (
        roi_ids_tab["ROI"].str.replace("L_", "").str.replace("_ROI", "")
    )
...
def get_voxel_ids(mask):
    ...
    """
    mask_array = mask.get_fdata()
    voxel_ids = np.array(np.where(mask_array.astype(bool))).T
    return voxel_ids


def get_voxel_ids_from_dict(mask_dict):
    """
    Apply get_voxel_ids() to each mask in a dictionary
    and create a dictionary for respective voxel IDs.
    """
    voxel_ids_dict = {}
...
def atlas_from_freesurfer_masks(
    sub, mask_dir, roi_ids, rh_constant=200, overwrite=False
):
    """
        ...
        for roi in roi_ids.keys():
            for hemi in ["L", "R"]:
                mask_raw = nifti1.load(
                    os.path.join(mask_dir, hemi_dict[hemi] + roi + "_ROI.fbr.nii.gz")
                )
                if len(mask_array) == 0:
                    mask_array_tmp = mask_raw.get_fdata()
                    mask_array_tmp[mask_array_tmp == 1] = (
                        roi_ids[roi][0] + hemi_add[hemi]
                    )  # convert binary mask by assigning roi_id to masked voxels
